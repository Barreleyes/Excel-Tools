...
class SheetData:
    ...
    def __check_repeat(self, key, key_list):
        if key in key_list:
            data_checker.RaiseException.key_repeat()
        else:
            key_list.append(key)
    ...
    def __load_head_info(self):
        ...
        def head_info(col):
            ...
            for row in range(0, len(self.head_attribute_h)):
                common.Current.Cell.set(row, col)
    ...
    def head_info_check(self):
        if self.mainkey_col == None:
            data_checker.RaiseException.key_too_less()
    ...
    def __load_sheet_horizon(self):
            ...
            for col in range(col_start, col_end):
                if self.head_info[col]:
                    field_name, field_stage = self.head_info[col]['name'], self.head_info[col][
                        'stage']
                    common.Current.Field.name = field_name
        ...
        if self.subkey_col:
            ...
            for item in self.group_info:
                # 生成主键数据
                common.Current.Cell.set(item[0], self.mainkey_col)
                mainkey = common.Current.Document.mainkey = data_process.process_type[mainkey_type](
                    self.cells(item[0], self.mainkey_col))
                common.Current.Document.subkey = 'null'
                ...
                for item_sub in range(item[0], item[1]+1):
                    common.Current.Cell.set(item_sub, self.subkey_col)
                    subkey = common.Current.Document.subkey = data_process.process_type[subkey_type](
        ...
        # 读取单键表
            ...
            for item in range(len(self.head_attribute_h)-1, self.row_max):
                common.Current.Cell.set(item, self.mainkey_col)
                mainkey = common.Current.Document.mainkey = data_process.process_type[mainkey_type](
    ...
    def __load_sheet_vertical(self):
        ...
        common.Current.Document.mainkey = 'null'
        common.Current.Document.subkey = 'null'
        for row in range(self.start_row_v-1, self.row_max):
            common.Current.Cell.set(row, col_name)
