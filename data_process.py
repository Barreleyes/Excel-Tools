...
def var_text(data, count=1):
    key = common.Current.key_str()
    localization.execute(
        "INSERT OR REPLACE into Localization (key,zh_CN) values ('%s','%s')" % (key, str(data)))
    localization.commit()
...
def dictionary(data, count=1):
    ...
    for element in data.split(dict_spliter_d1):
        ...
        field_name = int(part.group(2)) if part.group(
            2).isdigit() else part.group(2)
